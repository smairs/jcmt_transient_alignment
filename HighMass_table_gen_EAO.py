...
def make_table(regions=['DR21C'],alignment_iteration=0)
    ...
    for region in REGIONS.keys():
        ...
        keyarray = np.array(list(data['450']['header']['airmass'].keys()))
    
        JD = np.array(list(data['450']['header']['julian_date'].values()),dtype=str)[np.argsort(keyarray)].T
        #key = np.array(list(data['450']['header']['airmass'].keys()), dtype=str)[np.argsort(keyarray)].T
        airmass = np.array(list(data['450']['header']['airmass'].values()), dtype=float)[np.argsort(keyarray)].T
        tau225 = np.array(list(data['450']['header']['t225'].values()), dtype=float)[np.argsort(keyarray)].T
    
        elev = np.array(list(data['450']['header']['elevation'].values()), dtype=float)[np.argsort(keyarray)].T
        ...
        key = np.array(sorted(list(data['450']['header']['airmass'].keys())), dtype=str).T
